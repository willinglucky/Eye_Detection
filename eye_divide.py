...
def show_image(img, facepoint, bboxs, headpose, id_num):
    ...
    for faceNum in range(0,facepoint.shape[0]):
        ...
        print(img.shape)
        img = img[int(down_min):int(top_max),int(round(facepoint[faceNum,36*2])-5):int(round(facepoint[faceNum,45*2])+5)]
...
def predictImage(dir_path):
    ...
    for file_name in file_list:
        ...
        show_image(colorImage, level1Point, bboxs, predictpose, index)
        index = index + 1
